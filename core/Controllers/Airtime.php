<?php
    ...
    class Airtime extends ApiAccess{
		...
		public function purchaseMyAirtime($body,$networkDetails){
            ...
            $host = trim(self::getConfigValue($details,$networkname.$name."Provider"));
            $apiKey = trim(self::getConfigValue($details,$networkname.$name."Key"));

            
            //Check If API Is Is Using N3TData Or Bilalsubs
            if(strpos($host, 'vtunaija') !== false){
            ...
            $err = curl_error($curl);
            return [$err, $body,$networkDetails];
